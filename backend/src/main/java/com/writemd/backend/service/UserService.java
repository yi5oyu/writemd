...
public class UserService {
    ...
    public NoteDTO noteContent(Long noteId) {
        Texts texts = textRepository.findByNotesIdWithNote(noteId)
            .orElseThrow(() -> new EntityNotFoundException("노트 찾을 수 없음"));

        NoteDTO note = NoteDTO.builder()
            .noteId(noteId)
            .noteName(texts.getNotes().getNoteName())
            .createdAt(texts.getNotes().getCreatedAt())
            .updatedAt(texts.getNotes().getUpdatedAt())
