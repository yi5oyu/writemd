...
export const templateCreateDuration = new Trend('api_template_create_duration')
export const memoCreateDuration = new Trend('api_memo_create_duration')

const DUMMY_TEXT = '## 부하 테스트\nK6 2번 시나리오 작성 시간: ' + new Date().toISOString()
...
export const options = {
  ...
  thresholds: {
    ...
    api_template_create_duration: ['p(95)<500'],
    api_memo_create_duration: ['p(95)<500'],
    errors: ['rate<0.01'],
  },
}
...
export default function () {
  ...
  if (rand < 0.1) {
    ...
    // 2.5%: 템플릿 조회/생성
    group('4. 템플릿 조회 및 생성', function () {
      const templatePayload = JSON.stringify({
        title: `Load Test Folder`,
        template: [
          {
      ...
      const res = http.post(`https://api.writemd.space/api/template/${githubId}`, templatePayload, {
        ...params,
        tags: { name: 'CreateTemplate' },
      })

      const success = check(res, { 'Template Created 201': (r) => r.status === 201 })
      errorRate.add(!success)
      if (success) templateCreateDuration.add(res.timings.duration)
    })
  } else {
    // 2.5%: 메모 조회/생성
    group('5. 메모 조회 및 생성', function () {
      const memoPayload = JSON.stringify({ text: `Load Test Memo - ${new Date().toISOString()}` })
      const res = http.post(`https://api.writemd.space/api/memo/${githubId}`, memoPayload, {
        ...params,
        tags: { name: 'CreateMemo' },
      })

      const success = check(res, { 'Memo Created 200': (r) => r.status === 200 })
      errorRate.add(!success)
      if (success) memoCreateDuration.add(res.timings.duration)
    })
  }
  sleep(Math.random() * 2 + 1)
}
